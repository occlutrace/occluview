...
fn ensure_test_runtime_dir() {
    ...
    let runtime_dir = RUNTIME_DIR.get_or_init(|| {
        let dir = std::env::temp_dir().join("occluview-shell-wgpu-runtime");
        let create = std::fs::create_dir_all(&dir);
        assert!(create.is_ok(), "create shell test runtime dir: {create:?}");

        let permissions = std::fs::set_permissions(&dir, std::fs::Permissions::from_mode(0o700));
        assert!(
            permissions.is_ok(),
            "set shell test runtime dir permissions: {permissions:?}"
        );
        dir
    });
